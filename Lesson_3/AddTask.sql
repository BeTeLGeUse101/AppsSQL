...
SELECT u.id, u.firstname, u.lastname, COUNT(m.filename) AS count_media
FROM users AS u
JOIN media AS m ON u.id = m.user_id
WHERE m.filename LIKE "%.doc" OR m.filename LIKE "%.docx" OR m.filename LIKE "%.html"
GROUP BY u.id, u.firstname, u.lastname;

-- Посчитать лайки для моих документов (моих медиа)
SELECT u.id, u.firstname, u.lastname, SUM(l.media_id) AS sum_likes
FROM users AS u
